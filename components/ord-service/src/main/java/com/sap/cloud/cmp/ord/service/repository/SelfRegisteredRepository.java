...
public interface SelfRegisteredRepository extends JpaRepository<RootEntity, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT value->>0 FROM labels where app_id = uuid(?1) AND key = ?2")
    String findApplicationTenantByLabelKey(String appId, String labelKey);
}