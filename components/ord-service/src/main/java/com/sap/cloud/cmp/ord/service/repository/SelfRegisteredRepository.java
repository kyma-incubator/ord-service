...
public interface SelfRegisteredRepository extends JpaRepository<RootEntity, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT f.id FROM formations f WHERE (SELECT l.value FROM labels l WHERE l.key = 'scenarios' and l.runtime_context_id = uuid(?1)) ? f.name AND f.formation_template_id IN (SELECT ft.id FROM formation_templates ft WHERE ft.discovery_consumers ? (SELECT l.value->>0 FROM labels l WHERE l.key = 'runtimeType' AND l.runtime_id = (SELECT rtctx.runtime_id FROM runtime_contexts rtctx where rtctx.id = uuid(?1))))")
    ...
    @Query(nativeQuery = true, value = "SELECT f.id FROM formations f WHERE (SELECT l.value FROM labels l WHERE l.key = 'scenarios' and l.app_id = uuid(?1)) ? f.name AND f.formation_template_id IN (SELECT ft.id FROM formation_templates ft WHERE ft.discovery_consumers ? (SELECT l.value->>0 FROM labels l WHERE l.key = 'applicationType' AND l.app_id = uuid(?1)))")
